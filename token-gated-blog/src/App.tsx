...
function App() {
  ...
  const fetchNonce = async () => {
    try {
      ...
      setNonceTime(nonceData.nonce_time)
    } catch (error) {
  ...
  const signMessage = async () => {
    if (!account.addresses?.length) return alert('Please connect wallet first')
    if (!nonce) return alert('Please fetch nonce first')

    const address = account.addresses[0]

    // SIWE message template (MUST match backend exactly)
    const finalMessage = `packtpub.com wants you to sign in with your Ethereum account:
${address}
...
- https://github.com/PacktPublishing/Hands-On-Blockchain-for-Python-Developers--2nd-Edition`
  ...
  return (
    <>
      ...
      <div>
        <h2>Nonce</h2>
        <button onClick={fetchNonce} type="button">
          Get Nonce
        </button>
        <div>Nonce: {nonce}</div>
      </div>
